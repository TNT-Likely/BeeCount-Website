...
def test_embedding_model():
    ...
    try:
        ...
        print(f"✓ Single text encoding passed (dimension: {len(embedding)})")
        ...
        print(f"✓ Batch encoding passed (encoded {len(texts)} texts)")
        
        # 测试相似度计算
        similarity = model.compute_similarity(embedding, embedding)
        assert similarity == 1.0, "Similarity of same vector should be 1.0"
        
        print(f"✓ Similarity computation passed (self-similarity: {similarity})")
...
def test_vector_store_rebuild():
    ...
    try:
        ...
        print("✓ Vector store rebuild test passed\n")
...
def test_mcp_vector_tools():
    ...
    try:
        ...
        print(f"✓ MCP vector tools test passed\n")
        
    except Exception as e:
        print(f"✗ MCP vector tools test failed: {e}")
        raise
