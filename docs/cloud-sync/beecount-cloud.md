...
| 用户类型 | 推荐理由 |
|---------|---------|
| 🏠 家庭多人记账 | 每人注册一个账号 + **共享账本**邀请家人共记一本,Owner / Editor 角色权限分明 |
| 📱 单人多设备 | 手机 / iPad / Web 三端秒级同步,不用手动"上传下载" |
| 🖥️ 有 NAS / VPS 的用户 | Docker 一键跑起来,数据在自己机器上 |
| 👨‍💻 技术用户 | 开源、可审计,想二次开发也可以 |

:::tip 版本升级提示(1.6.0)
- App 3.6.0 的[账本多币种](../account/multi-currency.md)(账本内交易记原币 + 账本统计折算到账本主币种)依赖 **Cloud ≥ 1.6.0**:服务端投影新增币种 / 折算两列、账本维度统计按折算口径汇总、Web 改金额时联动重算折算。升级顺序**务必先服务端、后 App**
- **本版含数据库 schema 迁移**(交易表新增 `currency_code` / `native_amount` 两列 + 历史数据回填),随镜像启动自动执行,`docker compose pull && docker compose up -d` 即可,无需手工步骤
- 老服务端不识别这两列,新 App 记的外币交易在 Web / 跨设备会表现为折算丢失、账本统计按原币裸加;升级到 1.6.0 即对齐
:::

:::tip 版本升级提示(1.5.2)
- App 3.5.2 的「明细备注显示方式」与[货币扩充至全量 ISO 4217(151 种)](../account/multi-currency.md)均为**纯客户端 / 前端特性,App 端不依赖服务端版本**;只是 Web 端要拿到同款体验(明细备注显示方式 + 151 种币种)需升级 **Cloud ≥ 1.5.2**
- **本版无数据库 schema 迁移**(备注显示方式走 appearance JSON 透传,币种是前端列表),`docker compose pull && docker compose up -d` 即可
:::

:::tip 版本升级提示(1.5.1)
- App 3.5.1 的[「账单标记」](../record/flags.md)需要 **Cloud ≥ 1.5.1**:云模式下,服务端的收支/预算汇总按「不计入收支」「不计入预算」标记过滤;老服务端不识别标记,云端统计会把已标记的交易也算进去(本地模式与单机统计不受影响,始终正确)。升级顺序**务必先服务端、后 App**
:::

:::tip 版本升级提示(1.5.0)
- App 3.5.0 的[多币种](../account/multi-currency.md)与[净值趋势](../account/net-worth-trend.md)依赖 **Cloud ≥ 1.5.0**:服务端提供主币种同步、手动汇率、汇率代理以及 net-worth-history 读端点。升级顺序**务必先服务端、后 App**
...
:::tip 版本升级提示(1.4.0)
- App 3.4.0 的[「每月起始日」](../account/month-start-day.md)需要 **Cloud ≥ 1.4.0**:老服务端会忽略该设置,表现为"设置不跨设备"。升级顺序建议**先服务端、后 App**
- 升级到 1.4.0 时数据库迁移自动执行,并会**自动回填修复**历史交易"账户统计漏算"的问题(老版本 Web 写入的部分交易缺账户关联,导致 Web 账户统计偏小),无需手工操作
...
每个用户默认数据独立、互相隔离。**多人协作场景**走"共享账本":Owner 在 App / Web 上一键生成邀请码,Editor 输入后即可加入。
